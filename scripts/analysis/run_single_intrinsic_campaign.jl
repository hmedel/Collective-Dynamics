"""
    run_single_intrinsic_campaign.jl

Ejecuta una sola simulación para la campaña INTRINSIC (arc-length collision detection).

Uso:
    julia --project=. run_single_intrinsic_campaign.jl <run_id> <N> <e> <a> <b> <seed> <max_time> <dt_max> <dt_min> <save_interval> <campaign_dir>

Recibe TODOS los parámetros desde la línea de comandos (no lee de CSV).
Usa detección de colisiones intrínseca y energy projection.
"""
...
if length(ARGS) < 11
    println("""
    Uso: julia run_single_intrinsic_campaign.jl <run_id> <N> <e> <a> <b> <seed> <max_time> <dt_max> <dt_min> <save_interval> <campaign_dir>
...
# Radio como fracción del semi-eje menor (5% de b)
...
open(log_file, "w") do io
    println(io, "="^80)
    println(io, "INTRINSIC CAMPAIGN RUN")
    println(io, "="^80)
...
# Ajustar max_attempts según N (alta densidad requiere más intentos)
...
try
    ...
    @printf("DONE: run_id=%d N=%d e=%.2f seed=%d ΔE/E₀=%.3e\n", run_id, N, e, seed, ΔE_rel_max)
    exit(0)
